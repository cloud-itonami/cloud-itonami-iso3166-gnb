(ns marketentry.store
  "SSoT for the GNB market-entry compliance actor, behind a `Store`
  ...
  PLUS `:requires-nif?`/`:nif-verified?`): the Guinea-Bissau dossier
  (`marketentry.facts`) grounds only Conservatória do Registo Comercial
  commercial registration via the CFE single window
  (`:requires-gnb-registration?`/`:has-gnb-registration?`), not a
  distinct tax-ID/corporate-number scheme -- so there is no GNB analog
  of `:requires-nif?`/`:nif-verified?` here (mirroring
  `cloud-itonami-iso3166-stp`/`-sdn`). See `marketentry.governor` for
  the corresponding check-count honesty note."
...
(defn demo-data
  ...
  5 engagements exercises a distinct governor path:
    ...
    eng-2 -- commercial-registration-missing (requires but lacks
             Conservatória/CFE registration)
    eng-3 -- engagement-fee-mismatch (claimed-fee != base + months x rate)
    eng-4 -- clean, different fee shape (a second honest commit path)
    eng-5 -- clean, does not itself require GNB registration (a
             pure-foreign-market-facing engagement -- the conditional
             branch of the flagship check)"
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Bolama Import Export Lda" :portal "Lei-Quadro Relativo Aos Contratos Públicos filing (no verified e-procurement portal)"
             :base-fee 320000 :monthly-rate 18000 :monitoring-months 12
             :claimed-fee 536000.0
             :requires-gnb-registration? true :has-gnb-registration? true
             :drafted? false :submitted? false
             :jurisdiction "GNB" :status :intake}
    "eng-2" {:id "eng-2" :operator "Cacheu Logistics" :portal "Lei-Quadro Relativo Aos Contratos Públicos filing (no verified e-procurement portal)"
             :base-fee 320000 :monthly-rate 18000 :monitoring-months 12
             :claimed-fee 536000.0
             :requires-gnb-registration? true :has-gnb-registration? false
             :drafted? false :submitted? false
             :jurisdiction "GNB" :status :intake}
    "eng-3" {:id "eng-3" :operator "Gabu Civic Works" :portal "Lei-Quadro Relativo Aos Contratos Públicos filing (no verified e-procurement portal)"
             :base-fee 320000 :monthly-rate 18000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-gnb-registration? true :has-gnb-registration? true
             :drafted? false :submitted? false
             :jurisdiction "GNB" :status :intake}
    "eng-4" {:id "eng-4" :operator "Bissau Cashew Traders" :portal "Lei-Quadro Relativo Aos Contratos Públicos filing (no verified e-procurement portal)"
             :base-fee 180000 :monthly-rate 12000 :monitoring-months 6
             :claimed-fee 252000.0
             :requires-gnb-registration? true :has-gnb-registration? true
             :drafted? false :submitted? false
             :jurisdiction "GNB" :status :intake}
    "eng-5" {:id "eng-5" :operator "Oio Regional Supply" :portal "Lei-Quadro Relativo Aos Contratos Públicos filing (no verified e-procurement portal)"
             :base-fee 150000 :monthly-rate 10000 :monitoring-months 3
             :claimed-fee 180000.0
             :requires-gnb-registration? false :has-gnb-registration? false
             :drafted? false :submitted? false
             :jurisdiction "GNB" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-gnb-registration? has-gnb-registration?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-gnb-registration?)    (assoc :engagement/requires-gnb-registration? requires-gnb-registration?)
    (some? has-gnb-registration?)         (assoc :engagement/has-gnb-registration? has-gnb-registration?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-gnb-registration? :engagement/has-gnb-registration?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-gnb-registration? (boolean (:engagement/requires-gnb-registration? m))
     :has-gnb-registration? (boolean (:engagement/has-gnb-registration? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-gnb"})
