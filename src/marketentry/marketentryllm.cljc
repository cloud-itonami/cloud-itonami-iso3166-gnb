(ns marketentry.marketentryllm
  ...
  the GNB public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-gnb-registration?=" (:has-gnb-registration? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-gnb-registration? e))
                              (:has-gnb-registration? e)))
                   0.9 0.3)}))
