(ns marketentry.governor-contract-test
  ...
  This is the GNB actor's SIX-check governor (see `marketentry.governor`
  docstring for why it is six, not the AGO reference's seven): there is
  no `nif-unverified`/tax-ID analog test here because there is no such
  check -- the dossier does not ground a distinct GNB tax-ID scheme.
  `commercial-registration-missing` is the GNB analog of AGO's
  `ao-entity-missing`/MLI's `rccm-entity-missing` flagship check."
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Bolama Import Export Lda"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Bolama Import Export Lda" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest commercial-registration-missing-is-held-and-unoverridable
  (testing "missing Conservatória/CFE commercial registration -> HARD hold (GNB flagship check)"
    (let [[db actor] (fresh)
...
(deftest engagement-not-requiring-registration-still-commits-end-to-end
  (testing "eng-5 does NOT itself require GNB registration -- the conditional branch of the flagship check still commits clean"
    (let [[db actor] (fresh)
          _ (assess! actor "t13pre" "eng-5")
          _ (draft! actor "t13pre" "eng-5")
          r1 (exec-op actor "t13" {:op :filing/submit :subject "eng-5"} operator)
          r2 (approve! actor "t13")]
      (is (= :interrupted (:status r1)))
      (is (= :commit (get-in r2 [:state :disposition])))
